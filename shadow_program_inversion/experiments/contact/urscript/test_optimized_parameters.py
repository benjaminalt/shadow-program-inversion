...
def main(args):
    ...
    optimized_param_filepath = os.path.join(cfg.OPTIMIZED_PARAM_DIR, f"urscript_{args.material}_{args.target_force}N.npy")
    ...
    _, _, _, real_tensor = load_data_file(test_data_filepath)
    for i, optimized_params in enumerate(optimized_parameters):
        ...
        vel = param_tensor[7].item()
        acc = param_tensor[8].item()
        real_trajectory = execute_move_linear(cfg.ROBOT_IP, rec, scenario.approach_pose(), goal_pose, vel, acc)
        ...
        if args.show_plots:
            plot_multiple_trajectories_vertical([Trajectory.from_tensor(real_tensor[i]),
                                                ...
                                                colors=["black", "red"], labels=["Before", "Optimized"])
        result_trajectories.append(real_trajectory)
    result_trajectories = torch.stack([traj.to_tensor(pad_to=TRAJECTORY_LENGTH) for traj in result_trajectories])
    output_filepath = os.path.join(cfg.RESULTS_DIR, f"urscript_{args.material}_{args.target_force}N.npy")
